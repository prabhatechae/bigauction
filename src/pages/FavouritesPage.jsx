...
function FavouriteCard({ auction, onRemove }) {
  const product   = auction.product
  const isLive    = auction.status === 'ACTIVE'
  const isInactive = auction.status === 'CLOSED' || auction.status === 'SOLD'
  ...
  return (
    <div className={`group bg-white border rounded-xl overflow-hidden transition-all duration-300 flex flex-col ${
      ...
      <Link to={`/products/${product?.id}`} className="block">
        <div className="aspect-[3/2] bg-taupe/10 overflow-hidden relative">
          ...
          {product?.conditionGrade && (
            <span className="absolute top-3 right-3 text-xs bg-almost-black/70 text-ivory/90 px-2 py-0.5 rounded-full backdrop-blur-sm">
              {product.conditionGrade === 'LIKE_NEW' ? 'Like New'
                : product.conditionGrade === 'EXCELLENT' ? 'Excellent'
                : product.conditionGrade === 'VERY_GOOD' ? 'Very Good'
                : product.conditionGrade === 'GOOD' ? 'Good'
                : 'Fair'}
            </span>
          )}
        </div>
      ...
      <div className="p-3 flex flex-col gap-1.5 flex-1">
        <div>
          <p className="text-gold text-xs font-semibold uppercase tracking-[0.15em]">{product?.brand}</p>
          ...
          <p className="text-taupe/60 text-xs mt-0.5" style={{ minHeight: '1rem' }}>
            {product?.sourceCountry ? `From ${product.sourceCountry}` : ''}
          </p>
        </div>
        ...
        <div className="border-t border-taupe/10 pt-2 flex items-end justify-between">
          <div>
            <p className="text-taupe text-[11px] uppercase tracking-wide mb-0.5">
              {isLive ? 'Current Bid' : auction.status === 'PENDING' ? 'Ticket Price' : 'Final Bid'}
            </p>
            <p className={`font-semibold text-sm ${isLive ? 'text-gold font-bold' : 'text-charcoal'}`}>
              {isLive
                ...
                ? (auction.ticketPrice ? `AED ${Number(auction.ticketPrice).toLocaleString()}` : 'TBA')
...
export default function FavouritesPage() {
  ...
  useEffect(() => {
    if (auctions.length === 0) dispatch(fetchAuctions())
  }, [])

  const saved = auctions.filter(a => favourites.includes(a.id))

  if (loading && auctions.length === 0) return <Loader text="Loading…" />
    ...
    <div>
      ...
      <div className="relative bg-emerald overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-20">
          <div className="flex flex-wrap items-center gap-3 mb-2">
            <h1 className="font-display text-ivory text-3xl sm:text-4xl font-semibold">Saved Auctions</h1>
            {saved.length > 0 && (
          ...
          <p className="text-ivory/70 text-base">Your favourite auctions in one place</p>
        </div>
      </div>
      ...
      {favourites.length === 0 ? (
        <div className="text-center py-24">
          ...
          <h3 className="font-display text-charcoal text-xl font-semibold mb-2">No saved auctions</h3>
          <p className="text-taupe text-sm mb-6">Tap the heart icon on any auction to save it here.</p>
          <Link
            to="/auctions"
        ...
        /* Saved IDs exist but auctions not found (stale) */
        <div className="text-center py-24">
          <div className="text-6xl text-taupe/20 mb-6">◆</div>
          <h3 className="text-charcoal font-semibold mb-2">Your saved auctions are no longer available</h3>
          <p className="text-taupe text-sm mb-6">They may have ended or been removed.</p>
          <Link to="/auctions" className="text-gold text-sm hover:underline">Browse new auctions →</Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {saved.map((auction, i) => (
            <div key={auction.id} className="animate-card-enter" style={{ animationDelay: `${i * 0.05}s` }}>
              <FavouriteCard auction={auction} onRemove={toggle} />
            </div>
          ))}
        </div>
      )}
